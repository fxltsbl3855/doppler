...
public class QueryServiceImpl extends HbaseUtils implements QueryService {
    ...
    public JSONArray getTracesByEx(String serviceId, Long startTime, int sum) {
        JSONArray array = new JSONArray();
        Scan scan = new Scan();
        scan.setStartRow(new String(serviceId + ":" + startTime + ":exception.dubbo".getBytes()).getBytes());
        scan.setStopRow(new String(serviceId + ":" + Long.MAX_VALUE + ":exception.dubbo".getBytes()).getBytes());
        Filter filter = new PageFilter(sum);
        scan.setFilter(filter);
        HTableInterface table = null;
        ResultScanner rs = null;
