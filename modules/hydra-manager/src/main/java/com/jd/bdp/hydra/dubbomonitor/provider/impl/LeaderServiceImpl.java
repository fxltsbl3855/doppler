...
public class LeaderServiceImpl implements LeaderService {
    ...
    @Override
    public Map<String, String> registerClient(String name, List<String> services , Map<String, Integer> sRoleMap) {
        HashMap<String, String> map = new HashMap<String, String>();
        map.put("seed", seedService.getSeed().toString());
        map.put(name, appService.getAppId(name).toString());
        for (String serviceName : services) {
            Integer sR = sRoleMap.get(name);
            map.put(serviceName, serviceService.getServiceId(serviceName, name, sR==null?-1:sR.intValue()).toString());
        }
        return map;
    }

    @Override
    public String registerClient(String name, String service, int serviceInvokeRole) {
        return serviceService.getServiceId(service, name,serviceInvokeRole).toString();
    }
