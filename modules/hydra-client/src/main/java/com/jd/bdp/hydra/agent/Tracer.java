...
public class Tracer {
    ...
    //同级传traceId
    private ThreadLocal<TraceBo> levelTraceThreadLocal = new ThreadLocal<TraceBo>();
    ...
    public void setLevelTrace(TraceBo traceBo) {
        levelTraceThreadLocal.set(traceBo);
    }
    public TraceBo getLevelTrace() {
       return levelTraceThreadLocal.get();
    }
    ...
    //构件rootSpan,是否采样
    public Span newSpan(String spanname, Endpoint endpoint, String serviceId,String traceId) {
        boolean s = isSample();
        Span span = new Span();
        Long tid = TracerUtils.getAttachmentLong(traceId);
        if(tid == null){
            tid = genTracerId();
            logger.debug("[hydra] generate traceId,tid="+tid);
        }
        span.setTraceId(s ? tid : null);
...
//        }
    ...
    public static void startTraceWork() {
        try {
            ...
            logger.error("start tracer fail,"+e.getMessage());
        }
    }
    ...
    public String getServiceId(String name, int serviceInvokeRole) {
        String id = null;
        if (transfer != null) {
            id = transfer.getServiceId(name, serviceInvokeRole);
        }else{
            logger.error("[hydra] getServiceId transfer is null,may be hydra-config.xml load error");
        }
        return id;
    }
    ...
    public void setInvokeInfo(String appName){
        transfer.setInvokeInfo(appName);
    }
}
